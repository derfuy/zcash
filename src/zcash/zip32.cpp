...
typedef std::array<unsigned char, ZC_DIVERSIFIER_SIZE> DiversifierIndex;
...
boost::optional<std::pair<DiversifierIndex, libzcash::SaplingPaymentAddress>>
    SaplingExtendedFullViewingKey::Address(DiversifierIndex j) const
{
    CDataStream ss_xfvk(SER_NETWORK, PROTOCOL_VERSION);
    ss_xfvk << *this;
    CSerializeData xfvk_bytes(ss_xfvk.begin(), ss_xfvk.end());

    DiversifierIndex j_ret;
